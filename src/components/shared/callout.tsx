...
interface CalloutProps {
  twClass?: string; // Optional additional Tailwind CSS class
  children?: React.ReactNode; // Children nodes to render
  type?: keyof typeof dataCallout; // Type of callout
}

// Callout data with corresponding icons and classes
const dataCallout = {
  default: {
    icon: Info,
    classes:
...
export function Callout({
  ...
  const { icon: Icon, classes } = dataCallout[type]; // Get icon and classes based on type
  return (
    <div
      className={cn(
        "mt-6 flex items-start space-x-3 rounded-lg border px-4 py-3 text-[15.6px] dark:border-none",
        classes, // Apply the specific classes
        twClass // Allow for additional classes
      )}
      {...props}
    >
      ...
      <div className="[&>p]:my-0">{children}</div> {/* Render children with no margin */}
    </div>
  );
}